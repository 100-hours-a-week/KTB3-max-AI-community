...
import cv2 #OpenCV - 시각적인 처리
from ultralytics import YOLO #YOLO모델

# 카메라로 받은 이미지를 프레임단위로 분석
# YOLO11n 모델을 사용하여 'mouse' 객체를 탐지 
# 탐지된 객체에 대해 경고 메시지를 출력
...
class ObjectDetector:
    ...
    def process_frame(self, frame: np.ndarray) -> Tuple[np.ndarray, Dict[str, Union[bool, Optional[Tuple[int, int]]]]]:
        """
        이미지 프레임을 분석하여 객체 탐지 후 각각의 프레임을 반환
        `mouse`가 탐지되었을 경우 경고 메시지를 추가

        이때 `mouse`가 탐지되었는지, 그때의 중심 좌표는 무엇인지 딕셔너리를 추가 반환
        """
        if self.model is None: #모델이 없으면
            return frame, {"detected": False, "center": None} # 원본 이미지 프레임을 그대로 반환

        # 프레임 분석
        results = self.model.predict(frame, conf=0.45, verbose=False) #신뢰도 45%, 분석 결과 터미널에 출력하지 않음
        r = results[0] #첫번째 결과만 가져오기
        
        mouse_detected = False #`mouse`를 탐지했는지 플래그
        center_point = None #`mouse` 박스의 중심 좌표
        ...
        if r.boxes is not None: #탐지된 객체가 있으면 아래 반복문 실행
            for box in r.boxes: #탐지된 각 객체들에 대하여 
                ...
                if class_name == 'mouse': #`mouse`가 탐지된 경우
        ...
        if mouse_detected: #`mouse`가 탐지된 경우
