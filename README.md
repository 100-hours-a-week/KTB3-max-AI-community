...
3. FastAPI를 사용하니 프론트엔드를 전혀 몰라도 파이썬만으로 통신이 가능하게 할 수 있다. 게다가 그 사용법도 쉬운편이여서 쉽게 체화 가능하다. 이번 과제에선 프론트엔드를 전혀 몰라 생성형AI에게 전권을 위임하였지만, 그 무지가 백엔드의 무지로 이어지지 않았다. FastAPI를 사용하면 프론트엔드에 대해 알지 못하여도 충분히 API통신이 가능하다는 것이 신기했다.

4. 따라서 백엔드와 그것을 이루는 사용자 정의 함수를 설계하고 익히는 과정이 지금 나에겐 제일 중요한 부분이다. 당장 이것에 대해 학습을 시작하도록 하겠다.